...
- [ ] **Step 2: Find the return statement in Pricing.jsx and add `<SEO>` as the first child**

  Locate the JSX return in the Pricing component (it returns a top-level `<div>` or `<>` fragment). Add `<SEO>` as the very first element inside the return:
  ...
  Note: If the return currently starts with a plain `<div>`, wrap with a fragment `<>...</>` and add SEO before the div.
...
- [ ] **Step 3: Verify the Pricing page now generates a canonical tag**
  ...
  ```
  Open `http://localhost:5173/pricing` in a browser, open DevTools → Elements, and confirm the `<head>` contains:
  ```html
...
- [ ] **Step 3: Deploy and submit sitemap to GSC**

  After pushing to production, go to Google Search Console → Sitemaps and re-submit `https://sapa.services/sitemap.xml` to trigger a fresh crawl.
